...
const Buses = () => {
    ...
    const handleCreateBus = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setNewBus({ busNumber: '', plateNumber: '', capacity: '' });
            setIsEditMode(false);
            setEditingBus(null);
    ...
    const handleToggleStatus = async (bus: any) => {
        ...
        try {
            await updateBus(bus._id, { status: newStatus });
            fetchBuses();
        } catch (err: any) {
                                    ...
                                    {filteredBuses.length > 0 ? (
                                        filteredBuses.map((bus) => (
                                            <tr key={bus._id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-slate-600">{bus.capacity || 0} seats</td>
                                                <td className="px-6 py-4">
                    ...
                    >
                        ...
                        >
                            ...
                            <form onSubmit={handleCreateBus} className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Capacity (Seats)</label>
                                    <input
                                        type="number"
                                        required
                                        min="1"
